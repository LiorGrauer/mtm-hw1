...
bool addPlayer(Map players, int player_id){
    ...
    RETURN_VALUE_IF_TRUE(!player,false);
    int* player_id_temp = malloc(sizeof(*player_id_temp));
    if(!player_id_temp){
        freePlayer((MapDataElement)player);
        return false;
    }
...
double calculateLevel (Player player, int total_games){
    return ((CHESS_WIN_SCORE_FOR_LEVEL*(player->wins)+
            CHESS_LOSE_SCORE_FOR_LEVEL*(player->losses)+
            CHESS_DRAW_SCORE_FOR_LEVEL*(player->draws))/(double)total_games);
}