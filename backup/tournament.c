...
void tournamentAddPlayers(Tournament tournament, int first_player, int second_player, WinnerPlayer winner){
    if (!mapContains(tournament->players, (MapKeyElement)(&first_player))){
        addPlayer(tournament->players, first_player);
        tournamentChangeTotalPlayers(tournament, 1);
    }
    if (!mapContains(tournament->players, (MapKeyElement)(&second_player))){
        addPlayer(tournament->players, second_player);
        tournamentChangeTotalPlayers(tournament, 1);
    }
