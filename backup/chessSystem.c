...
static void chessAddPlayers(Map players, int first_player, int second_player, Winner winner, int play_time){
    if (!mapContains(players, (MapKeyElement)(&first_player))){
        addPlayer(players, first_player);
    }
    if (!mapContains(players, (MapKeyElement)(&second_player))){
        addPlayer(players, second_player);
    }
...
ChessResult chessAddGame(ChessSystem chess, int tournament_id, int first_player,
    ...
    *game_id = mapGetSize(games)+1;
    ...
    RETURN_VALUE_IF_TRUE(result == MAP_OUT_OF_MEMORY,CHESS_OUT_OF_MEMORY);
    assert(result == MAP_SUCCESS);
    chessAddPlayers(chess->players, first_player, second_player, winner, play_time);
    tournamentAddPlayers(tournament, first_player, second_player, winner);
    return CHESS_SUCCESS;
}
...
//removes inactive players from the players map
//recieves the players map and deletes inactive players
static void chessRemoveInactivePlayers(Map players){
    if (mapGetSize(players) == 0){
        return;
    }
    MapKeyElement player_id = mapGetFirst(players);
    MapKeyElement next_id = NULL;
    while (player_id){
        Player player = mapGet(players, player_id);
        if (playerGetTotalGames(player) == 0){
            next_id = mapGetNext(players);
    ...
    MAP_FOREACH(int*, tournament_id, chess->tournaments){
        ...
        MAP_FOREACH(int*, game_id, games){
            ...
            if (player_id == gameGetFirstPlayerID(game)){
                gameChangeFirstPlayerID(game, -1);
            ...
            if (player_id == gameGetSecondPlayerID(game)){
                gameChangeSecondPlayerID(game, -1);
...
ChessResult chessSavePlayersLevels (ChessSystem chess, FILE* file){
    ...
    MAP_FOREACH(int*, player_id, chess->players){
        ...
        if (player_total_games > 1){
            curr_level = calculateLevel(player,player_total_games);
            if (!mapContains(levels, (MapKeyElement)(&curr_level))){
                ...
                mapPut(levels, (MapKeyElement)(&curr_level), (MapDataElement)players);
                mapDestroy(players);
            }
            else{
...
ChessResult chessSaveTournamentStatistics (ChessSystem chess, char* path_file){
    ...
    MAP_FOREACH (int*, tournament_id, chess->tournaments){
        ...
        free(tournament_id);
        if (tournamentGetWinnerID(tournament) > 0){
            fprintf(file, "%d\n%d\n%.2f\n%s\n%d\n%d\n", tournamentGetWinnerID(tournament), calculateLongestPlayTime(tournament), 
            calculateaveragePlayTime(tournament), tournamentGetLocation(tournament), mapGetSize(tournamentGetGames(tournament)), 
